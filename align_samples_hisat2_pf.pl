...
open (IN, "/local/projects-t3/SerreDLab-3/kieran.tebben/mali/sample_lists/sample1.txt");
my @sample_names;
...
##Local
##system("/usr/local/packages/hisat2-2.0.4/hisat2 -x $genome -1 $R1_filepath -2 $R2_filepath -S  $sam --no-unal -p 16 --max-intronlen 5000 &> $log"); 
